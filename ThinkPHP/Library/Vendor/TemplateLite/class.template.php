...
class Template_Lite {
	...
	var $plugins_dir = array (
			"plugins" 
	); // where the plugins are to be found
	...
	function fetch($file, $cache_id = null, $display = false) {
		...
		if (strpos ( $output, $this->_sl_md5 ) !== false) {
			...
			foreach ( $_match [1] as $value ) {
				...
				$output = str_replace ( $this->_sl_md5 . '{_run_insert ' . $value . '}' . $this->_sl_md5, call_user_func_array ( 'insert_' . $arguments ['name'], array (
						( array ) $arguments,
						$this 
				) ), $output );
			}
	...
	function _fetch_compile($file) {
		...
		} else if ($this->_resource_type == "file") {
			$f = fopen ( $file, "r" );
			$size = filesize ( $file );
			if ($size > 0) {
				$file_contents = fread ( $f, $size );
	...
	function _run_insert($arguments) {
		if ($this->cache) {
			...
			if (isset ( $arguments ['assign'] )) {
				$this->assign ( $arguments ['assign'], call_user_func_array ( 'insert_' . $arguments ['name'], array (
						( array ) $arguments,
						$this 
				) ) );
			} else {
				return call_user_func_array ( 'insert_' . $arguments ['name'], array (
						( array ) $arguments,
						$this 
				) );
			}

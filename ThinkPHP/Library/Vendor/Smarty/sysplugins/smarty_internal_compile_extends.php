...
class Smarty_Internal_Compile_Extends extends Smarty_Internal_CompileBase {
	...
	public function compile($args, $compiler) {
		...
		$compiler->template->properties ['file_dependency'] [$template_sha1] = array (
		...
		while ( $_start + 1 < $_result_count ) {
			...
			$_block_content = str_replace ( $compiler->smarty->left_delimiter . '$smarty.block.parent' . $compiler->smarty->right_delimiter, '%%%%SMARTY_PARENT%%%%', substr ( $_content, $_result [0] [$_start] [1] + strlen ( $_result [0] [$_start] [0] ), $_result [0] [$_start + $_end] [1] - $_result [0] [$_start] [1] - + strlen ( $_result [0] [$_start] [0] ) ) );
		...
		if ($_template->source->type == 'extends') {
			...
			foreach ( $_template->source->components as $key => $component ) {
				$compiler->template->properties ['file_dependency'] [$key] = array (
						$component->filepath,
						$component->timestamp,
						$component->type 
				);

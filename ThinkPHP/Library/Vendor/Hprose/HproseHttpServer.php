...
class HproseHttpServer {
	...
	public function __filterHandler($data) {
		if (preg_match ( '/<b>.*? error<\/b>:(.*?)<br/', $data, $match )) {
			...
			$data = HproseTags::TagError . HproseFormatter::serialize ( trim ( $error ), true ) . HproseTags::TagEnd;
		}
		if ($this->filter)
	...
	private function doInvoke() {
		...
		do {
			...
			$args = array ();
			$byref = false;
			$tag = $simpleReader->checkTags ( array (
					HproseTags::TagList,
					HproseTags::TagEnd,
					HproseTags::TagCall 
			) );
	...
	public function addMethod($methodname, $belongto, $alias = NULL, $resultMode = HproseResultMode::Normal, $simple = NULL) {
		...
		if (is_string ( $belongto )) {
			$this->addFunction ( array (
					$belongto,
					$methodname 
			), $alias, $resultMode, $simple );
		} else {
			$this->addFunction ( array (
					&$belongto,
					$methodname 
			...
			case 2 :
				{
					if (is_callable ( $args [0] ) && is_string ( $args [1] )) {
						...
						if (is_string ( $args [1] ) && ! is_callable ( array (
								$args [1],
								$args [0] 
						) )) {
	...
	public function handle() {
		...
		if ($this->filter)
			$HTTP_RAW_POST_DATA = $this->filter->inputFilter ( $HTTP_RAW_POST_DATA );

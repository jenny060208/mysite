...
class HproseRawReader {
	...
	private function readNumberRaw($ostream, $tag) {
		$s = $tag . $this->stream->readuntil ( HproseTags::TagSemicolon ) . HproseTags::TagSemicolon;
		$ostream->write ( $s );
	}
	private function readDateTimeRaw($ostream, $tag) {
		...
		do {
			$tag = $this->stream->getc ();
			$s .= $tag;
		} while ( $tag != HproseTags::TagSemicolon && $tag != HproseTags::TagUTC );
		$ostream->write ( $s );
	}
	...
	private function readComplexRaw($ostream, $tag) {
		$s = $tag . $this->stream->readuntil ( HproseTags::TagOpenbrace ) . HproseTags::TagOpenbrace;
		$ostream->write ( $s );
...
class HproseSimpleReader extends HproseRawReader {
	...
	public function readDate($includeTag = false) {
		if ($includeTag)
			$this->checkTag ( HproseTags::TagDate );
		...
		if ($tag == HproseTags::TagTime) {
			...
			if ($tag == HproseTags::TagUTC) {
				$date = new HproseDateTime ( $year, $month, $day, $hour, $minute, $second, $microsecond, true );
			} else {
				$date = new HproseDateTime ( $year, $month, $day, $hour, $minute, $second, $microsecond );
			}
	...
	public function readTime($includeTag = false) {
		if ($includeTag)
			$this->checkTag ( HproseTags::TagTime );
		$hour = ( int ) ($this->stream->read ( 2 ));
		$minute = ( int ) ($this->stream->read ( 2 ));
		$second = ( int ) ($this->stream->read ( 2 ));
		$microsecond = 0;
	...
	public function readBytes($includeTag = false) {
		if ($includeTag)
			$this->checkTag ( HproseTags::TagBytes );
	...
	public function readString($includeTag = false) {
		if ($includeTag)
			$this->checkTag ( HproseTags::TagString );
	...
	public function readGuid($includeTag = false) {
		if ($includeTag)
			$this->checkTag ( HproseTags::TagGuid );
		$this->stream->skip ( 1 );
	...
	public function &readList($includeTag = false) {
		if ($includeTag)
			$this->checkTag ( HproseTags::TagList );
	...
	public function &readMap($includeTag = false) {
		if ($includeTag)
			$this->checkTag ( HproseTags::TagMap );

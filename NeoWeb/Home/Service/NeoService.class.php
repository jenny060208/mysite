...
use Home\Logic\TagRegisterLogic;
...
class NeoService extends Service
{
     ...
     * Description: scan tag process result
     */
    public function tagScanEventLog($setData)
    {
        $neoLogic = new ScanTagProcessLogic();
        $result = $neoLogic->scanEventLog($setData);
     ...
     * Description: scan tag process result
     */
    public function tagScanErrorEventLog($setData)
    {
        $neoLogic = new ScanTagProcessLogic();
        $result = $neoLogic->errorEventLog($setData);

        return ($result);
    }

    /**
     * Name : loadUserRegisterInfo
     * Access: public
     * Input :$tid -- tag id
     *
     * Output: tag related web page info
     *
     * Description: get the tag related webpage info for user register
     */
    public function loadUserRegisterInfo($tid)
    {
        $mLogic = new TagRegisterLogic();
        $result = $mLogic->loadUserRegisterInfo($tid);

        if ($result->getStatus()) {
            // Load the business name and reward message
            $retVal = $mLogic->loadTagMerchantInfo($result->getBusinessId());
        }
    ...
    /**
     * Name : addTagUserRegisterInfo
     * Access: public
     * Input :$recv_data -- user register data in json format
     *
     * Output: process result
     *
     * Description: get the tag related webpage info for user register
     */
    public function addTagUserRegisterInfo($recv_data)
    {
        $mLogic = new TagRegisterLogic();
        $result = $mLogic->addUserRegisterInfoFromTag($recv_data);

        return ($result);
    }

    /**
     * Name : loadUserRegistSuccessInfo
     * Access: public
     * Input : none
     * Output: result of user regist success info through tag scan
     *
     * Description: load the user regist success info through tag scan
     */
    public function loadUserRegistSuccessInfo()
    {
        $mLogic = new TagRegisterLogic();
        $result = $mLogic->getUserRegistSuccessInfo();
